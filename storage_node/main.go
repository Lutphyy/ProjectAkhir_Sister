...
func (n *Node) dataPathFor(fileID string) string {
	if len(fileID) < 2 { return filepath.Join(n.DataDir, fileID) }
	sub := fileID[:2]
	dir := filepath.Join(n.DataDir, sub)
	_ = os.MkdirAll(dir, 0755)
	return filepath.Join(dir, fileID)
}
func (n *Node) addUsed(delta int64){ n.mu.Lock(); n.usedBytes += delta; if n.usedBytes<0{n.usedBytes=0}; n.mu.Unlock() }
...
func (n *Node) handleUpload(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseMultipartForm(32<<20); err != nil { http.Error(w,"parse form",400); return }
	fileID := r.FormValue("fileId"); if fileID=="" { http.Error(w,"missing fileId",400); return }
	f, hdr, err := r.FormFile("file"); if err!=nil { http.Error(w,"missing file",400); return }
	defer f.Close()

	target := n.dataPathFor(fileID)
	out, err := os.Create(target); if err!=nil { http.Error(w,"cannot create",500); return }
	defer out.Close()

	h := sha256.New()
	size, err := copyWithHash(out, f, h); if err!=nil { http.Error(w,"write error",500); return }
	n.addUsed(size)
...
func (n *Node) handleDownload(w http.ResponseWriter, r *http.Request) {
	...
	if fileID=="" { http.Error(w,"missing fileId",400); return }
	path := n.dataPathFor(fileID)
	f, err := os.Open(path); if err!=nil { http.Error(w,"not found",404); return }
	defer f.Close()
	http.ServeContent(w, r, fileID, time.Now(), f)
}

func (n *Node) handleHas(w http.ResponseWriter, r *http.Request) {
	fileID := r.URL.Query().Get("fileId"); if fileID=="" { http.Error(w,"missing fileId",400); return }
	_, err := os.Stat(n.dataPathFor(fileID))
	writeJSON(w, map[string]any{"exists": err==nil})
}
func (n *Node) handleHealth(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, map[string]any{"nodeId":n.NodeID,"status":"HEALTHY","usedBytes":n.currentUsed()})
}
...
func postJSON(url string, body any) error {
	...
	defer resp.Body.Close(); io.ReadAll(resp.Body); return nil
}
...
func main() {
	...
	if v := getenv("CAPACITY_BYTES",""); v!="" { var x int64; fmt.Sscanf(v,"%d",&x); if x>0 { node.CapacityBytes=x } }
